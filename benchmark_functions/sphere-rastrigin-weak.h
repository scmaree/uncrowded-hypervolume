...
namespace hicam
{
  ...
  class sphereRastriginWeak_t : public fitness_t
  {
  ...
  public:
    ...
    sphereRastriginWeak_t(double A)
    {
    ...
    void define_problem_evaluation(solution_t & sol)
    {
      ...
      sol.obj[1] /= (A + optimum_x0_shift*optimum_x0_shift);
    ...
    void define_problem_evaluation_with_gradients(solution_t & sol)
    {
      ...
      sol.gradients[1][0] /= (A + optimum_x0_shift*optimum_x0_shift);
      
      for(size_t i = 1; i < sol.param.size(); ++i) {
        sol.gradients[1][i] = 2 * sol.param[i]; // + 2 * PI * A * sin(2*PI*(sol.param[i] - optimum_x0_shift));
        sol.gradients[1][i] /= (A + optimum_x0_shift*optimum_x0_shift);
      }
