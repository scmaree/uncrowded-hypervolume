/*
 ...
 S.C. Maree, T. Alderliesten, P.A.N. Bosman, PPSN-2020.
...
{
  printf("Usage: bezea [-?] [-P] [-s] [-w] [-v] [-n] [-e] [-r] pro dim cpts ssize low upp pop ela eva sec vtr rnd wrp\n"); // [-f]
  ...
  printf(" cpts: Number of control points for the bezier curve (Bezier curve order).\n");
...
void parseParameters(int argc, char **argv, int *index)
{
  ...
  noError = noError && sscanf(argv[*index + 2], "%zd", &number_of_reference_points); // bezier control points
  noError = noError && sscanf(argv[*index + 3], "%zd", &number_of_test_points); // points in the solution set, i.e., points on the bezier curve
...
void checkOptions(void)
{
  ...
  if(mo_fitness_function->partial_evaluations_available) {
    // exploit partial evaluations if possible
    local_optimizer_index = 63;
  } else {
    // we use a full
  ...
  {
    printf("\n");
    printf("Error: number of MO parameters <= 0 (read: %zu). Require MO number of parameters >= 1.", mo_number_of_parameters);
    printf("\n\n");
    
    exit(0);
  }

  if (number_of_reference_points <= 1)
  {
    printf("\n");
    printf("Error: Number of Bezier control points <= 1 (read: %zu). Require >= 1.", number_of_reference_points);
    printf("\n\n");
  ...
  {
    printf("\n");
    printf("Error: solution set size <= 0 (read: %zu). Require >= 1.", number_of_test_points);
    printf("\n\n");
...
int main(int argc, char **argv)
{
  ...
  if(!use_bezier_interpolation) {
    fitness_function = std::make_shared<hillvallea::UHV_t>(mo_fitness_function, number_of_reference_points, collect_all_mo_sols_in_archive, elitist_archive_size_target, nullptr, use_finite_differences);
  } else {
    fitness_function = std::make_shared<hillvallea::bezierUHV_t>(mo_fitness_function, number_of_reference_points, number_of_test_points, collect_all_mo_sols_in_archive, elitist_archive_size_target, nullptr);
  }
  ...
  if(print_verbose_overview)
  {
    ...
    std::cout << "pareto_sets = [\n";
    for(size_t i = 0; i < opt.elitist_archive.size(); ++i) {
      std::cout << "\t" << opt.elitist_archive[i]->param << " " << opt.elitist_archive[i]->f << std::endl;
    }
    std::cout << " ];\n";
    ...
    for(size_t i = 0; i < opt.best.mo_test_sols.size(); ++i) {
      // approximation_set.updateArchive(sol.mo_test_sols[i]);
      approximation_set.addSolution(opt.best.mo_test_sols[i]);
    }
    ...
    double smoothness = approximation_set.computeSmoothness();
    
    std::cout << "Smoothness = " << std::scientific << std::setw(10) << std::setprecision(4) << smoothness << "\n";
  }
}
