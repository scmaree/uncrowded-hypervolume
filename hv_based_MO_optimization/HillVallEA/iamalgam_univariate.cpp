...
void hillvallea::iamalgam_univariate_t::initialize_from_population(population_pt pop, size_t target_popsize)
{
  this->pop = pop;
  multiplier = 1.0;
...
void hillvallea::iamalgam_univariate_t::generation(size_t sample_size, int & number_of_evaluations)
{
...
bool hillvallea::iamalgam_univariate_t::checkTerminationCondition()
{
  ...
  if (terminate_for_param_std_mean_zero || terminate_on_parameter_std || terminate_on_fitness_std || terminate_on_distribution_multiplier || terminate_on_slow_improvement || terminate_on_penalty_std)
  {
     active = false;
     return !active;
  }
